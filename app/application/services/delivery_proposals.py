...
class DeliveryProposalService(IDeliveryProposalService):
    ...
    async def notify_order_assigned(self, order_id: UUID) -> Notification | None:
        ...
        if not order:
            logger.warning(
                "Order not found for delivery assignment notification",
        ...
        if not assignment or not assignment.employee_id:
            logger.warning(
                "No agent assigned to retail point for order notification",
                order_id=str(order_id),
        ...
        if order.planned_visit_id:
            next_plan = await self._uow.visit_plans.get_by_id(order.planned_visit_id)
    ...
    async def plan_order_delivery(self, order_id: UUID) -> Order | None:
        ...
        if not order:
            logger.warning(
                "Order not found for delivery planning", order_id=str(order_id)
            )
            return None
        ...
        if order.status in (
            ...
            logger.warning(
                "Order cannot be planned in its current status",
                order_id=str(order_id),
        ...
        if not assignment or not assignment.employee_id:
            logger.warning(
                "No agent assigned to retail point for order delivery planning",
                order_id=str(order_id),
                retail_point_id=str(order.retail_point_id),
            )
            return None
        ...
        logger.info(
            "Order planned for delivery visit",
            order_id=str(order.id),
            planned_visit_id=str(next_plan.id),
            plan_date=str(next_plan.plan_date),
        )

        await self.notify_order_assigned(order.id)
        return order
