...
@pytest.mark.asyncio
async def test_worker_run_route_generation_handles_exception(mock_config, mock_route_service):
...
def test_worker_start(mock_config, mock_route_service):
    ...
    with patch.object(worker._scheluder, "add_job") as mock_add_job, \
...
def test_worker_stop(mock_config, mock_route_service):
    ...
    with patch.object(type(worker._scheluder), "running", new_callable=PropertyMock, return_value=True), \
...
def test_worker_stop_when_not_running(mock_config, mock_route_service):
    ...
    with patch.object(type(worker._scheluder), "running", new_callable=PropertyMock, return_value=False), \
         patch.object(worker._scheluder, "shutdown") as mock_shutdown:
        worker.stop()
